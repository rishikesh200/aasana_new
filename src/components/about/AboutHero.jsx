import React, { useEffect, useState } from 'react';
import api from '../../services/api';

const AboutHero = () => {
  const [data, setData] = useState({
    aboutBanner: '',
    bannerHeading: '',
    bannerSubHeading: '',
  });

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get('/about/hero');
        setData(res.data);
      } catch (error) {
        console.error('Error fetching About Hero data:', error);
      }
    };
    fetchData();
  }, []);

  return (
    <section className='relative w-full h-[80vh]'>
      <div 
        className="w-full h-full relative overflow-hidden" 
        style={{ 
          backgroundImage: data?.aboutBanner ? `url(${data.aboutBanner})` : 'none',
          backgroundRepeat: "no-repeat", 
          backgroundSize: "cover", 
          backgroundPosition: "center"
        }}
      >
          {/* Overlay */}
          <div className='absolute inset-0 flex flex-col justify-center items-center'>
            <div 
              className='rounded-2xl px-6 py-6 sm:px-10 sm:py-12 md:px-16 md:py-16 lg:px-20 lg:py-20 xl:px-24 xl:py-24 w-[80%] sm:w-[85%] md:w-[75%] lg:w-[65%] xl:w-[60%] max-w-4xl text-center backdrop-blur-md border border-white/30 shadow-2xl md:mt-30' 
              style={{
                background: 'linear-gradient(to right, rgba(173, 216, 230, 0.3), rgba(255, 182, 193, 0.3))'
              }}
            >
              <h1 className='font-bold font-[Raleway] font-light text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl 2xl:text-8xl text-white mb-4 sm:mb-5 md:mb-6'>
                {data?.bannerHeading || 'About Us'}
              </h1>
              <p className='font-[Raleway] font-light text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl 2xl:text-3xl text-white'>
                {data?.bannerSubHeading || 'Finding Inspiration in Every Turn'}
              </p>
            </div>
          </div>
      </div>
    </section>
  );
};

export default AboutHero;
